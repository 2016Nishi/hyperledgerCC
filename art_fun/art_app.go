...
var aucTables = []string{"UserTable", "ItemTable", "ItemHistoryTable", "AuctionTable", "BidTable", "TransTable"}
...
type UserObject struct {
	...
	Name      string
	UserType  string // AUCTION HOUSE (AH), PERSON (P), BUSINESS (B)
	Address   string
	Phone     string
	Email     string
	Bank      string
...
func (t *SimpleChaincode) Init(stub *shim.ChaincodeStub, function string, args []string) ([]byte, error) {
	...
	err = InitItemReg(stub)
	if err != nil {
		return nil, fmt.Errorf("Initiation of Item Register Failed ")
	}
	err = InitItemMaster(stub)
	if err != nil {
		return nil, fmt.Errorf("Initiation of Item Master Failed ")
	}
	err = InitAucReg(stub)
	if err != nil {
		return nil, fmt.Errorf("Initiation of Auction Register Failed ")
	}
	err = InitTransReg(stub)
	if err != nil {
		return nil, fmt.Errorf("Initiation of Transaction Register Failed ")
	}
	err = InitUserReg(stub)
	if err != nil {
		return nil, fmt.Errorf("Initiation of User Register Failed ")
	}
	err = InitBidReg(stub)
	if err != nil {
		return nil, fmt.Errorf("Initiation of Bid Register Failed ")
	}
...
func UpdateItemObject(stub *shim.ChaincodeStub, ar []byte, hammerPrice string, buyer string) ([]byte, error) {
	...
	if err != nil {
		fmt.Printf("U() : UpdateItemObject() : Failed to create Transaction Object from JSON ")
		return nil, err
	}
	...
	//err = stub.PutState(myItem.ItemID, ar)
	keys := []string{myItem.ItemID, myItem.CurrentOwnerID}
	err = ReplaceLedgerEntry(stub, "ItemTable", keys, ar)
	if err != nil {
		fmt.Printf("UpdateItemObject() : Failed to insert Transaction Object into Blockchain ")
		return nil, err
	}
...
func PostTransaction(stub *shim.ChaincodeStub, function string, args []string) ([]byte, error) {
	...
	fmt.Printf("Auction House information  ", buyer, ar.UserId)
	if err != nil {
		fmt.Printf("PostTransaction() : Failed Buyer not Registered in Blockchain ", ar.UserId)
		return nil, err
	}

	fmt.Printf("PostTransaction() : Buyer info : ", buyer)
	...
	if err != nil {
		fmt.Printf("Transaction() : write error while inserting item log record\n")
		return lastUpdatedItemOBCObject, err
	}
	...
	fmt.Printf("PostTransaction() : Item info Authenticated : ", ar.ItemID)
	...
	if err != nil {
		fmt.Printf("PostTransaction() : Failed Cannot create object buffer for write : " + args[1] + "\n")
		return nil, errors.New("PostUser(): Failed Cannot create object buffer for write : " + args[1])
	} else {
		// Update the ledger with the Buffer Data
		//err = stub.PutState(args[0], buff)
...
func InitLedger(stub *shim.ChaincodeStub, tableName string) error {
	...
	for i := 0; i < nKeys; i++ {
		columnDef := shim.ColumnDefinition{Name: "keyName", Type: shim.ColumnDefinition_STRING, Key: true}
...
func GetLastBid(stub *shim.ChaincodeStub, function string, args []string) ([]byte, error) {
	...
	Avalbytes := rows[len(rows)-1].Columns[nCol].GetBytes()
	return Avalbytes, nil
}
...
func ShellCmdToCloseAuction(aucID string) error {
	...
	fileHandle, _ := os.Create("closeauction.sh")
	...
	fmt.Println("Kicking off offline CloseAuction", argStr)
	return nil
}
...
func CloseAuction(stub *shim.ChaincodeStub, function string, args []string) ([]byte, error) {
	...
	aucR.Status = "CLOSED"
	fmt.Println("OpenAuctionForBids(): UpdateAuctionStatus() successful ", aucR)
	...
	fmt.Println("OpenAuctionForBids(): UpdateAuctionStatus() successful ", Avalbytes)
	fmt.Println("OpenAuctionForBids(): Proceeding to process the highest bid ")
